...
class PersonalCenterScreen extends ConsumerWidget {
      ...
      body: SafeArea(
        ...
        child: RefreshIndicator(
          ...
          child: CustomScrollView(
            ...
            // VIP 等级卡片
            SliverToBoxAdapter(
              child: _buildVIPCard(context, user),
            ),
  ...
  Widget _buildVIPCard(BuildContext context, User? user) {
    ...
    if (levelData != null) {
      ...
      final currentCharge = double.tryParse((
        levelData.recharge ??
        user?.totalRecharge ??
        user?.totalDeposit ??
        user?.rechargeAmount ??
      ...
      final requiredCharge = double.tryParse((
        levelData.nextRecharge ?? 
        0
      ).toString()) ?? 0;

      final currentFlow = double.tryParse((
      ...
      final requiredFlow = double.tryParse((
        levelData.nextValidBetAmount ?? 
        0
      ).toString()) ?? 0;
      
      final isMaxLevel = requiredCharge <= 0 && requiredFlow <= 0 && levelData.nextVipLevel == null;
      ...
      if (isMaxLevel) {
        ...
        final rechargeProgress = requiredCharge > 0 ? (currentCharge / requiredCharge).clamp(0.0, 1.0) : 1.0;
        final betProgress = requiredFlow > 0 ? (currentFlow / requiredFlow).clamp(0.0, 1.0) : 1.0;
        progress = (rechargeProgress + betProgress) / 2;

        rechargeProgressText = '${currentCharge.toStringAsFixed(0)}/${requiredCharge.toStringAsFixed(0)}';
        validBetProgressText = '${currentFlow.toStringAsFixed(0)}/${requiredFlow.toStringAsFixed(0)}';
      }
    }
          ...
          Row(
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: const Row(
                  children: [
                    Text('等级权益', style: TextStyle(color: AppTheme.warning, fontSize: 12)),
                    Icon(Icons.chevron_right, color: AppTheme.warning, size: 16),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildWalletCard(BuildContext context, WidgetRef ref, User? user, UserState userState) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 20, vertical: 10),
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
  ...
  Widget _buildWalletAction(BuildContext context, String label, IconData icon, Color color, VoidCallback onTap) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildListItem(context, Icons.feedback_outlined, '意见反馈', '您的建议是我们进步的动力', () => context.push('/feedback'), iconColor: Colors.purple),
