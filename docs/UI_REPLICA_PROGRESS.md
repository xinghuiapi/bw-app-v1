...
- **底导架构** (`CustomTabBar`)：**【深度对齐】** 5 个主 Tab（首页、游戏大厅、活动、客服、我的）均已接入底部导航；Tab 点击使用 `context.go(...)` 替代 push，主 Tab 路由使用 `NoTransitionPage`，对齐 m1 `van-tabbar route replace` 的无感切换体验。
