...
| `/Users/john/Documents/trae_projects/bw-6-2/src/projects/m1` | UI 和页面逻辑来源 | 页面视觉意图、页面生命周期、接口入参、路由参数、跳转规则、状态处理、多语言 key；游戏一级分类图片布局以 `views/main/Game.vue` 的 `.provider-grid` / `.provider-cover` 为准 | 不直接复制 Vue template/CSS 重写 Flutter；不得用 bw-6-2 的其他子项目或旧 Flutter 项目替代 |
