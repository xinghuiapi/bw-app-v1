...
- 当前下一阶段开发重点为 P2：Telegram query 拦截已完成；后续继续邀请/refcode 持久化、`/system/getlist` terminal 参数、活动详情语言参数和部分模型字段联调校准。
...
- 账号：找回密码、Telegram 登录/设置密码形成真实提交闭环。
...
5. **当前联调边界**：Telegram 深链、邀请参数、系统 terminal、活动详情语言参数、DayRevenue/Withdraw/Telegram 字段等需要真实后端返回确认的内容。
6. **每步完成后同步文档**：更新接口顺序、业务化计划和 UI 进度，保持任务状态一致。
